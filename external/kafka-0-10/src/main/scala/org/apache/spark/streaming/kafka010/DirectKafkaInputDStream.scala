...
private[spark] class DirectKafkaInputDStream[K, V](
  ...
  protected def getPreferredHosts: ju.Map[TopicPartition, String] = {
    locationStrategy match {
      ...
      // TODO return back here to analyze what we can do better
      case PreferCachedConsumer => ju.Collections.emptyMap[TopicPartition, String]()
  ...
  override def compute(validTime: Time): Option[KafkaRDD[K, V]] = {
    ...
    val rdd = new KafkaRDD[K, V](context.sparkContext,
      executorKafkaParams, offsetRanges.toArray, getPreferredHosts, true, Some(locationStrategy))
